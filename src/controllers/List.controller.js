...
const addNewList = asynchandler(async (req, res) => {
  ...
  try {
    ...
    let Date, Month, Year, PrizeBondAmount, FirstWin, FirstPrize, SecondWin, SecondPrize, ThirdWin, ThirdPrize;
    ...
    console.log(
      "First Prize Number:",
      firstPrizeNumber.length > 0 ? firstPrizeNumber : "First Prize Number not found."
    );
    console.log("Second Win Prize ", SecondWinPrize);
    console.log(
      "Second Prize Numbers:",
      secondPrizeNumbers.length > 0 ? secondPrizeNumbers.join(", ") : "Second Prize Numbers not found."
    );
    console.log("Third Win Prize ", ThirdWinPrize);
    console.log(
      "All Prize Numbers:",
      allPrizeNumbers.length > 0 ? allPrizeNumbers.join(", ") : "All Prize Numbers not found."
    );
    console.log(
      typeof Date,
      typeof Month,
    ...
    if (!list) {
      return res.status(500).json(new ApiError(500, "Something Went Wrong with the Registration of User"));
    }

    return res.status(201).json(new ApiResponse(200, list, "List Saved Successfully"));
